...
def create_users():
    try:
        ...
        try:
            ...
            status = "CORRECTAS" if admin_auth else "INCORRECTAS"
            print(f'Admin: {admin_user.username}, Rol: {admin_user.role}, Credenciales: {status}')
        except User.DoesNotExist:
            print('Admin: No encontrado')
        
        # Verificar vendedor
        vendedor_auth = authenticate(username='vendedor1', password='Vendedor123!')
        status = "CORRECTAS" if vendedor_auth else "INCORRECTAS"
        print(f'Vendedor: {vendedor_user.username}, Rol: {vendedor_user.role}, Credenciales: {status}')
        
        # Verificar cliente
        cliente_auth = authenticate(username='cliente1', password='Cliente123!')
        status = "CORRECTAS" if cliente_auth else "INCORRECTAS"
        print(f'Cliente: {cliente_user.username}, Rol: {cliente_user.role}, Credenciales: {status}')
